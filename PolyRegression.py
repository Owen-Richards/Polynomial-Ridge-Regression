...
def get_loss_per_poly_order(x, y, degrees):
    # your code
    split_length = len(x) * 0.75
    x_train, x_validation = np.split(x, [int(split_length)])
    y_train, y_validation = np.split(y, [int(split_length)])

    training_losses = []
    validation_losses = []
    for degree in degrees:
        train_poly = increase_poly_order(x_train, degree)
        theta_train = normal_equation(train_poly, y_train)
        y_predict_train = predict(train_poly, theta_train)
        training_losses.append(get_loss(y_train, y_predict_train))

        poly_validation = increase_poly_order(x_validation, degree)
        y_predict_validation = predict(poly_validation, theta_train)
        validation_losses.append(get_loss(y_validation, y_predict_validation))
...
def get_loss_per_num_examples(x, y, example_num, train_proportion):
    ...
    for i in example_num:
        x_train, x_test, y_train, y_test = train_test_split(x[:i], y[:i], train_proportion)
